...
public class Esercizio_3_4 {
    ...
    public void soluzione(){
        System.out.println("Popolazione per continente (in millioni)");
        ...
        for (int i = 0; i < stati.length; i++) {
            ...
            int popolazioneTotale = 0 ;
            for (int j = 0; j < popolazione[i].length; j++) {
                System.out.printf("%-15s", popolazione[i][j]);
                popolazioneTotale += popolazione[i][j];
            }
            System.out.printf("%-15s", popolazioneTotale);
            System.out.println(" ");
